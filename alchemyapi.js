...
function AlchemyAPI() {
	...
	if (key.length != 40) {
		console.log('The API key in api_key.txt does not appear to be valid.');
		console.log('If you do not have a key, register for one at: http://www.alchemyapi.com/api/register.html');
		process.exit(1);
	}
	...
	//Set the key
	this.apikey = key;
//	console.log("API key read successfully. Using key: " + key + " to access Alchemy services");
...
AlchemyAPI.prototype.keywords = function(flavor, data, options, callback) {
	...
	if (!(flavor in AlchemyAPI.ENDPOINTS['keywords'])) {
		...
		//Add the data to the options and analyze
		options[flavor] = data;
		//console.log("calling alchemy endpoints, flavor: " + flavor + " with options: " + options);
